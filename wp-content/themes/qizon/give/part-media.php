<?php 
  $images = get_post_meta( get_the_ID(), 'qizon_give_gallery_images' , false );
  $video = get_post_meta( get_the_ID(), 'qizon_give_video_url' , true ); 
  $_random = wp_rand();
?>
	...
	<div class="campaign-media givewp-media">
		...
		<?php if($video){ ?>
		  	<div class="givewp-video">
		    	<a class="video-link popup-video" href="<?php echo esc_url($video) ?>"><i class="qicon-video-camera-2"></i></a>
		  	</div>
		<?php } ?>
