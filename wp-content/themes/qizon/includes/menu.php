...
class Qizon_Mega_Menu {
	...
	function menu_wp_edit_nav_menu_walker($class, $menu_id){
		return 'Qizon_Walker_Nav_Menu_Edit';
	}
...
class Qizon_Walker extends Walker {
	/**
 ...
 */
class Qizon_Walker_Nav_Menu_Edit extends Walker_Nav_Menu {
	/**
	 * Starts the list before the elements are added.
	 *
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( ! empty( $item->_invalid ) ) {
			...
			$title = sprintf( esc_html__( '%s (Invalid)' , 'qizon'), $item->title );
		} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
			$classes[] = 'pending';
			/* translators: %s: title of menu item in draft status */
			$title = sprintf( esc_html__('%s (Pending)', 'qizon'), $item->title );
		}
		...
		<li id="menu-item-<?php echo esc_attr($item_id); ?>" class="<?php echo implode(' ', $classes ); ?>">
			<dl class="menu-item-bar">
               ...
               <span class="menu-megamenu-handle"><?php echo esc_html__('Mega Menu', 'qizon'); ?></span>
					<label class="item-title" for="menu-item-checkbox-<?php echo esc_attr($item_id); ?>">
						...
						<span class="is-submenu <?php echo esc_attr($submenu_text_class) ?>"><?php echo esc_html__('Sub item', 'qizon'); ?></span>
					</label>
					<span class="item-controls">
						...
						<span class="item-order hide-if-js">
							...
							?>" class="item-move-up"><abbr title="<?php echo esc_attr__('Move up', 'qizon'); ?>">&#8593;</abbr></a>
							|
							<a href="<?php
								echo wp_nonce_url(
									add_query_arg(
							...
							?>" class="item-move-down"><abbr title="<?php echo esc_attr__('Move down', 'qizon'); ?>">&#8595;</abbr></a>
						</span>
						<a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>" title="<?php echo esc_attr__('Edit Menu Item', 'qizon'); ?>" href="<?php
			...
			<div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
         	...
         	<div class="menu-megamenu" style="border: 2px solid #000;">
             	<h4 class="menu-megamenu-title"><?php echo esc_html__( 'Mega Menu Setting', 'qizon' ); ?></h4>
             	<p class="menu-megamenu-enable">
                     ...
                     <?php echo esc_html__( 'Mega Menu Enabled', 'qizon' ); ?>
                  </label>
               </p>
                  ...
                  <label>
                     <?php 
                      ...
                      echo esc_html__( 'Number columns', 'qizon' ); ?><br />
                  	...
                  	<p><?php  echo esc_html__( 'Number columns of submenu(support 1,2,3,4,6 columns)', 'qizon' ); ?></p>
                  </label>
              	</p>
                  ...
                  <label>
                     <?php if(empty($item->megawidth) || $item->megawidth==0){
                   	...
                   	echo esc_html__( 'Mega menu width', 'qizon' ); ?><br />
               ...
               <p class="menu-megamenu-megaalign">
                  <label>
                     ...
                     echo esc_html__( 'Mega menu Align', 'qizon' ); ?><br />
                     <select id="edit-menu-item-megaalign-<?php echo esc_attr($item_id); ?>" class="width-75 code edit-menu-item-megaalign" name="menu-item-megaalign[<?php echo esc_attr($item_id); ?>]">
                     	<option value="fullwidth" <?php echo esc_attr($item->megaalign == 'fullwidth' ? 'selected' : '') ?>><?php echo esc_html__( "Full Width", 'qizon' ) ?></option>
                     	<option value="center" <?php echo esc_attr($item->megaalign == 'center' ? 'selected' : '') ?>><?php echo esc_html__( "Center", 'qizon' ) ?></option>
                     	<option value="left" <?php echo esc_attr($item->megaalign == 'left' ? 'selected' : '') ?>><?php echo esc_html__( "Left", 'qizon' ) ?></option>
                     	<option value="right" <?php echo esc_attr($item->megaalign == 'right' ? 'selected' : '') ?>><?php echo esc_html__( "Right", 'qizon' ) ?></option>
                     </select>
                  </label>
              	</p>
              	<p class="menu-megamenu-submegamenu">
              		<label>
              			<?php echo esc_html__( 'Mega Menu', 'qizon' ); ?><br/>
                  	...
                  	<p><?php  echo esc_html__( 'The ID of Elementor Template Section', 'qizon' ); ?></p>
            		</label>
            	</p>
            </div>
            ...
            <?php // custom field for mega menu end ?>
             	<div>
                     ...
                     <label>
                         <?php echo esc_html__( 'Icon', 'qizon' ); ?> <a href="<?php echo is_ssl() ? 'https' : 'http'; ?>://fortawesome.github.io/Font-Awesome/icons/" target="_bank"><?php esc_html_e('(icons in Font Awesome)', 'qizon'); ?></a> <br />
				...
				<?php if( 'custom' == $item->type ) : ?>
					<p class="field-url description description-wide">
						<label for="edit-menu-item-url-<?php echo esc_attr($item_id); ?>">
							<?php echo esc_html__( 'URL' , 'qizon'); ?><br />
				...
				<p class="description description-thin">
					<label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
						<?php echo esc_html__( 'Navigation Label' , 'qizon'); ?><br />
						<input type="text" id="edit-menu-item-title-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
					</label>
				</p>
				<p class="description description-thin">
					<label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
						<?php echo esc_html__( 'Title Attribute' , 'qizon'); ?><br />
				...
				<p class="field-link-target description">
					<label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
						...
						<?php echo esc_html__( 'Open link in a new window/tab' , 'qizon'); ?>
					</label>
				</p>
				<p class="field-css-classes description description-thin">
					<label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
						<?php echo esc_html__( 'CSS Classes (optional)' , 'qizon'); ?><br />
						<input type="text" id="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
					</label>
				</p>
				<p class="field-xfn description description-thin">
					<label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
						<?php echo esc_html__( 'Link Relationship (XFN)' , 'qizon'); ?><br />
						<input type="text" id="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
					</label>
				</p>
				<p class="field-description description description-wide">
					<label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
						<?php echo esc_html__( 'Description' , 'qizon'); ?><br />
						<textarea id="edit-menu-item-description-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr($item_id); ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
						<span class="description"><?php echo esc_html__('The description will be displayed in the menu if the current theme supports it.', 'qizon'); ?></span>
					</label>
				</p>

				<p class="field-move hide-if-no-js description description-wide">
					<label>
						<span><?php echo esc_html__( 'Move' , 'qizon'); ?></span>
						<a href="#" class="menus-move-up"><?php echo esc_html__( 'Up one' , 'qizon'); ?></a>
						<a href="#" class="menus-move-down"><?php echo esc_html__( 'Down one' , 'qizon'); ?></a>
						<a href="#" class="menus-move-left"></a>
						<a href="#" class="menus-move-right"></a>
						<a href="#" class="menus-move-top"><?php echo esc_html__( 'To the top' , 'qizon'); ?></a>
					</label>
				</p>
				...
				<div class="menu-item-actions description-wide submitbox">
					<?php if( 'custom' != $item->type && $original_title !== false ) : ?>
						<p class="link-to-original">
							<?php printf( esc_html__('Original: %s', 'qizon'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
						</p>
					<?php endif; ?>
					...
					); ?>"><?php echo esc_html__( 'Remove' , 'qizon'); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url( add_query_arg( array( 'edit-menu-item' => $item_id, 'cancel' => time() ), admin_url( 'nav-menus.php' ) ) );
						?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e('Cancel', 'qizon'); ?></a>
				</div>
...
$qizon_megamenu = new Qizon_Mega_Menu();
$qizon_megamenu->init_mega_menu();
