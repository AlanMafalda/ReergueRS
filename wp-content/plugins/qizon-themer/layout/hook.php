...
class GVA_Layout_Frontend_Hook extends GVA_Layout_Model{
   ...
   function __construct(){
      add_action('qizon/layouts/page', array($this, 'layout_page'));
      add_action('qizon/layouts/single/post', array($this, 'layout_single_post'));
      add_action('qizon/layouts/archive/post', array($this, 'layout_archive_post'));

      add_action('qizon/layouts/single/template', array($this, 'single_template'));

      add_action('qizon/layouts/single/crowdfunding', array($this, 'layout_single_crowdfunding'));
      add_action('qizon/layouts/archive/product', array($this, 'layout_archive_product'));

      add_action('qizon/layouts/single/give_form', array($this, 'layout_single_give_form'));
      add_action('qizon/layouts/archive/give_forms', array($this, 'layout_archive_give_forms'));
      
      add_action('wp_enqueue_scripts', array($this, 'qizon_themer_elementor_load_css'), 500);
   }

   public function qizon_themer_elementor_load_css(){
      ...
      if($post && $post->post_type == 'give_forms'){
         ...
         $post_meta_template = get_post_meta($post->ID, 'qizon_template_layout', true);
   ...
   public function layout_page(){
      ...
      if($post_meta_template == '_without_layout' || empty($post_meta_template) || !metadata_exists('post', $post_id, 'qizon_template_layout')){
   ...
   public function layout_single_give_form(){
      ...
      $post_id = $post->ID;
      $template_id = $this->get_template_default('givewp_layout');
      $post_template_id = get_post_meta($post_id, 'qizon_template_layout', true);
   ...
   public function layout_single_crowdfunding(){
      global $post;
      $post_id = $post->ID;
      $template_id = $this->get_template_default('single_crowdfunding_layout');
      $post_template_id = get_post_meta($post_id, 'qizon_template_layout', true);
      if( !empty($post_template_id) && $post_template_id && $post_template_id != '_default_active' && is_numeric($post_template_id) ){
         $template_id = $post_template_id;
      }
   ...
   public function layout_archive_product(){
      global $post;
      $template_id = $this->get_template_default('archive_crowdfunding_layout');
      if($template_id){
         $content_page = \Elementor\Plugin::instance()->frontend->get_builder_content_for_display($template_id);
         if($content_page){
            echo $content_page;
      ...
      do_action('qizon_page_breacrumb');
